...
create table public.transactions (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  -- Null only for transfers, which move money between wallets without a category.
  ...
  version integer not null default 1 check (version > 0),
  metadata jsonb not null default '{}'::jsonb,
  -- A transfer needs a destination wallet and no category; anything else is the reverse.
...
create unique index wallets_workspace_name_unique_idx on public.wallets(workspace_id, normalized_name) where deleted_at is null;
create index wallets_workspace_idx on public.wallets(workspace_id) where deleted_at is null;
-- At most one default wallet per workspace.
create unique index wallets_workspace_default_unique_idx on public.wallets(workspace_id) where is_default and deleted_at is null;
create index transactions_workspace_date_idx on public.transactions(workspace_id, transaction_at desc);
create index transactions_workspace_wallet_date_idx on public.transactions(workspace_id, wallet_id, transaction_at desc);
create index transactions_workspace_to_wallet_idx on public.transactions(workspace_id, to_wallet_id) where to_wallet_id is not null;
...
create trigger wallets_normalize_name_trigger before insert or update of name on public.wallets for each row execute function public.wallets_normalize_name();
...
create policy wallets_select_member on public.wallets for select using (public.is_workspace_member(workspace_id) and deleted_at is null);
create policy wallets_insert_manager on public.wallets for insert with check (public.can_manage_workspace(workspace_id));
create policy wallets_update_manager on public.wallets for update using (public.can_manage_workspace(workspace_id)) with check (public.can_manage_workspace(workspace_id));
...
create or replace function public.create_wallet_rpc(p_workspace_id uuid, p_name text, p_wallet_type text, p_icon_name text, p_color_value text, p_opening_balance_minor bigint default 0, p_currency_code text default 'VND', p_is_default boolean default false, p_sort_order integer default 0, p_id uuid default null)
returns public.wallets as $$
declare inserted public.wallets;
begin
  if not public.can_manage_workspace(p_workspace_id) then raise exception 'Only workspace owner or admin can create wallets'; end if;
  ...
  insert into public.wallets(id, workspace_id, name, normalized_name, wallet_type, icon_name, color_value, opening_balance_minor, currency_code, is_default, sort_order, created_by_user_id, updated_by_user_id)
  values (coalesce(p_id, gen_random_uuid()), p_workspace_id, trim(p_name), public.normalize_name(p_name), p_wallet_type, p_icon_name, p_color_value, p_opening_balance_minor, p_currency_code, p_is_default, p_sort_order, auth.uid(), auth.uid())
  returning * into inserted;
  return inserted;
end;
$$ language plpgsql security definer set search_path = public;

create or replace function public.update_wallet_rpc(p_wallet_id uuid, p_workspace_id uuid, p_name text, p_wallet_type text, p_icon_name text, p_color_value text, p_opening_balance_minor bigint, p_currency_code text, p_is_default boolean, p_is_archived boolean default false, p_sort_order integer default 0)
returns void as $$
begin
  if not public.can_manage_workspace(p_workspace_id) then raise exception 'Only workspace owner or admin can update wallets'; end if;
  ...
  update public.wallets set name = trim(p_name), normalized_name = public.normalize_name(p_name), wallet_type = p_wallet_type, icon_name = p_icon_name, color_value = p_color_value, opening_balance_minor = p_opening_balance_minor, currency_code = p_currency_code, is_default = p_is_default, is_archived = p_is_archived, sort_order = p_sort_order, updated_by_user_id = auth.uid(), updated_at = now(), version = version + 1
...
returns void as $$
begin
  if not public.can_manage_workspace(p_workspace_id) then raise exception 'Only workspace owner or admin can change the default wallet'; end if;
...
begin
  if not public.can_manage_workspace(p_workspace_id) then raise exception 'Only workspace owner or admin can delete wallets'; end if;
  ...
  update public.wallets set deleted_at = now(), deleted_by_user_id = auth.uid(), updated_by_user_id = auth.uid(), updated_at = now(), is_default = false, version = version + 1
  where id = p_wallet_id and workspace_id = p_workspace_id and deleted_at is null;
end;
$$ language plpgsql security definer set search_path = public;

create or replace function public.create_transaction_rpc(p_workspace_id uuid, p_category_id uuid, p_amount_minor bigint, p_currency_code text, p_transaction_type text, p_transaction_at timestamptz, p_wallet_id uuid, p_to_wallet_id uuid default null, p_note text default null, p_client_reference_id text default null, p_allow_overdraft_override boolean default false, p_id uuid default null, p_created_at timestamptz default now(), p_updated_at timestamptz default now())
returns public.transactions as $$
declare inserted public.transactions;
begin
  insert into public.transactions(id, workspace_id, category_id, amount_minor, currency_code, transaction_type, transaction_at, wallet_id, to_wallet_id, note, client_reference_id, created_by_user_id, updated_by_user_id, created_at, updated_at)
  values (coalesce(p_id, gen_random_uuid()), p_workspace_id, p_category_id, p_amount_minor, p_currency_code, p_transaction_type, p_transaction_at, p_wallet_id, p_to_wallet_id, p_note, p_client_reference_id, auth.uid(), auth.uid(), p_created_at, p_updated_at)
  returning * into inserted;
  return inserted;
end;
$$ language plpgsql security definer set search_path = public;

create or replace function public.update_transaction_rpc(p_transaction_id uuid, p_workspace_id uuid, p_category_id uuid, p_amount_minor bigint, p_currency_code text, p_transaction_type text, p_transaction_at timestamptz, p_wallet_id uuid, p_to_wallet_id uuid default null, p_note text default null, p_allow_overdraft_override boolean default false, p_updated_at timestamptz default now())
returns void as $$
begin
  update public.transactions set category_id = p_category_id, amount_minor = p_amount_minor, currency_code = p_currency_code, transaction_type = p_transaction_type, transaction_at = p_transaction_at, wallet_id = p_wallet_id, to_wallet_id = p_to_wallet_id, note = p_note, updated_by_user_id = auth.uid(), updated_at = p_updated_at, version = version + 1
