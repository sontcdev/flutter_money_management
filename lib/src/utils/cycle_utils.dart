...
class CycleUtils {
  ...
  static ({DateTime start, DateTime end}) getCurrentCycleRange(int monthStartDay) {
    return getCycleRangeForDate(DateTime.now(), monthStartDay);
  }

  /// Lấy khoảng thời gian chu kỳ chứa ngày [date]
  static ({DateTime start, DateTime end}) getCycleRangeForDate(DateTime date, int monthStartDay) {
    // Đảm bảo monthStartDay trong khoảng hợp lệ
    final startDay = monthStartDay.clamp(1, 28); // Clamp to 28 to avoid month overflow issues
    ...
    if (date.day >= startDay) {
      ...
      cycleEnd = DateTime(nextMonth.year, nextMonth.month, startDay - 1, 23, 59, 59);
  ...
  static ({DateTime start, DateTime end}) getCycleRangeForMonth(DateTime month, int monthStartDay) {
